...
var positions = {
    ...
    "RWB": "Right Winger Back", "LWB": "Left Winger Back",
    "CM": "Central Mid Fielder", "CDM": "Defensive Mid Fielder",
    "CAM": "Attacking Mid Fielder", "RM": "Right Mid Fielder",
    "LM": "Left Mid Fielder", "ST": "Striker", "CF": "Center Forward",
    "RW": "Right Winger", "LW": "Left Winger"
};

var t_positions = {
    "CB": 0, "RB": 0, "LB": 0, "RWB": 0, "LWB": 0,
    "CM": 2, "CDM": 2, "CAM": 2, "RM": 2, "LM": 2,
    "ST": 1, "CF": 1, "RW": 1, "LW": 1,
    "Gk": 3, "Attacker": 1, "Mid Fielder": 2,
    "Goal Keeper": 3, "Defence": 0, "Players": 5
};
...
function plotSunBurst(rootData) {
    d3.selectAll("#sunburstplot").remove();
    sunburst_tooltip.style("opacity", 0);

    var wrapper = d3.select("#sunburst");
    let width = wrapper.node().getBoundingClientRect().width;
    let height = wrapper.node().getBoundingClientRect().height;
    ...
    d3.partition()(root);  // Adds x0, x1, y0, y1
    ...
    nodes.append("rect")
        ...
        .attr("fill", d => d.data.name === "Players" ? "#4A6FA5" : color[t_positions[d.data.name]])
        .attr("stroke", "#fff")
        .on("click", d => handleClick(d))
        .on("mouseover", d => mousemove(d))
        .on("mouseleave", d => mouseleave(d));
    ...
    function mousemove(d) {
        sunburst_tooltip
            ...
            .html((positions[d.data.name] || d.data.name) + " " + formatNumber(d.value));
    }
}
